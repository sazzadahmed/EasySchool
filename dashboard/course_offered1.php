...
<form action="print_amount.php" method="post">
<table class="table">
       <tr>
         <th>SL</th>
         <th>Course Code</th>
...
while (list($key, $val) = each($_POST['course_offer_id'])){
    ...
    ?>
       ...
       <?php
       $query = $db->query("SELECT * FROM `course_list` inner join amount on amount.credit=course_list.credit where course_list.id='$course_offer_id'");
         if($query->num_rows>0) {
